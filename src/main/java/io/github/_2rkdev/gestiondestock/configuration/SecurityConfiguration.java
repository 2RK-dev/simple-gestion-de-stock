...
public class SecurityConfiguration {
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http, Converter<Jwt, ? extends AbstractAuthenticationToken> jwtAuthenticationConverter) {
        return http
    ...
    public JwtDecoder jwtDecoder(ResourceLoader resourceLoader, SecurityProperties securityProperties) {
        RSAPublicKey publicKey = readRsaPublicKey(securityProperties.jwt().publicKeyUri(), resourceLoader);
        return NimbusJwtDecoder.withPublicKey(publicKey).build();
    }

    @Bean
    @ConditionalOnExpression("!'${app.security.jwt.public-key-uri}'.isBlank() && !'${app.security.jwt.private-key-uri}'.isBlank()")
    public JwtEncoder jwtEncoder(ResourceLoader resourceLoader, SecurityProperties securityProperties) {
        RSAPublicKey publicKey = readRsaPublicKey(securityProperties.jwt().publicKeyUri(), resourceLoader);
        RSAPrivateKey privateKey = readRsaPrivateKey(securityProperties.jwt().privateKeyUri(), resourceLoader);
